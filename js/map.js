window.MapSystem = {
  render(){
    ...
    s.map.forEach((id,i)=>{
      ...
      if(id==='road'){ const x=i%s.mapSize,y=Math.floor(i/s.mapSize),at=(xx,yy)=>xx>=0&&yy>=0&&xx<s.mapSize&&yy<s.mapSize?s.map[yy*s.mapSize+xx]:null; const dirs={n:at(x,y-1)==='road',e:at(x+1,y)==='road',s:at(x,y+1)==='road',w:at(x-1,y)==='road'}; let active=Object.keys(dirs).filter(k=>dirs[k]); if(!active.length) active=['e','w']; const bend=active.length===2&&((active.includes('n')||active.includes('s'))&&(active.includes('e')||active.includes('w'))); tile.classList.add(...active.map(k=>'road-'+k)); tile.innerHTML=`<span class="tile-art road ${bend?'bend':''}">${active.map(k=>`<i class="road-arm ${k}"></i>`).join('')}<i class="road-core"></i>${active.map(k=>`<i class="lane-mark ${k}"></i>`).join('')}<i class="road-dot"></i></span>`; } else if(id){
        const exactStaticBuildings=['school','recycling','cinema','firestation','transitHub','hotel'];
        if(exactStaticBuildings.includes(id)) tile.innerHTML=`<img class="map-static-build-icon" src="assets/build-icons/${id}.svg" alt="" draggable="false">`;
        else tile.innerHTML=SpriteArt.markup(id);
        if(['workshop','factory','advanced'].includes(id)) tile.insertAdjacentHTML('beforeend','<i class="ambient-puff" aria-hidden="true"></i>'); if(['shops','mall'].includes(id)) tile.insertAdjacentHTML('beforeend','<i class="ambient-glint" aria-hidden="true"></i>'); }
