window.MapSystem = {
  ...
  applyViewMode(){
    const root=document.getElementById('cityMap');if(!root||!Game.state)return;
    ...
    if(!fit){root.style.removeProperty('--tile');root.style.removeProperty('height');root.style.removeProperty('min-height');if(prev!=='normal')setTimeout(()=>this.centerView(false),0);return;}
    const n=Game.state.mapSize||16,gap=2,pad=24;
    const maxW=Math.max(240,root.parentElement?.clientWidth||root.clientWidth||800)-pad;
    const maxH=Math.max(280,Math.min(window.innerHeight*.68,760))-pad;
    const tile=Math.max(8,Math.min(42,Math.floor(Math.min((maxW-gap*(n-1))/n,(maxH-gap*(n-1))/n))));
    root.style.setProperty('--tile',tile+'px');
    const h=tile*n+gap*(n-1)+pad;root.style.height=h+'px';root.style.minHeight='0px';
  ...
  async exportImage(){
    ...
    for(let y=0;y<n;y++)for(let x=0;x<n;x++){const id=s.map[y*n+x],px=pad+x*tile,py=pad+header+y*tile;ctx.fillStyle=((x+y)%3===0)?'#566f4c':'#526a48';ctx.fillRect(px,py,tile-1,tile-1);if(id){if(id==='road'){ctx.fillStyle='#3f4654';ctx.fillRect(px+1,py+1,tile-3,tile-3);ctx.fillStyle='#d9ca72';ctx.fillRect(px+3,py+Math.floor(tile/2),tile-7,2);}else{const img=iconCache.get(id);if(img)ctx.drawImage(img,px+2,py+2,tile-5,tile-5);}}}
